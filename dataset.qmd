...
We eliminated white spaces and renamed the variables to make it quicker to read, and more significant to interpret.
...
-   The missing observations represents 1.6% of the dataset.
...
# count_missing_obs <- colSums(is.na(train_data))
# as.data.frame(count_missing_obs)
# 
# # if the observation is greater than 0 display in the table
# if (count_missing_obs > 0) {
#   kbl(count_missing_obs) %>%
...
# Count the number of missing values in each column
count_missing_obs <- colSums(is.na(train_data))
count_missing_obs <- data.frame(count_missing_obs)
...
# ) 
# Create the kable object with styling options
count_missing_obs %>%
  kable() %>%
